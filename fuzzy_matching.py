...
class FuzzyMatcher:
    def __init__(self):
        # Step 5: Define matching terms universe
        ...
        # Low difference = Good Match (0 to ~10)
        ...
        # High difference = Bad Match (~18 to 30)
    ...
    def get_fuzzy_match_level(self, total_difference):
        ...
        return {
            "low": level_low,      # High Compatibility
            "medium": level_med,   # Medium Compatibility
            "high": level_high     # Low Compatibility
        }

    def visualize(self):
        import matplotlib.pyplot as plt
        plt.figure(figsize=(8, 5))
        plt.plot(self.x_diff, self.diff_low, 'g', linewidth=1.5, label='Low Diff (Good Match)')
        plt.plot(self.x_diff, self.diff_medium, 'b', linewidth=1.5, label='Medium Diff')
        plt.plot(self.x_diff, self.diff_high, 'r', linewidth=1.5, label='High Diff (Bad Match)')
